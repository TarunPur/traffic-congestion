...
## Phase 10 — Journey 2 live (screens 23, 22)

- **P10.1 — 23 Managed home** (`/managed`) ✅ — mirrors the J1 home; the assigned managed ride leads (Be-ready-by `SplitFlap` + countdown + board + `.tl.mgd` "Assigned" + reliability verdict). Start today's trip → `/trip`; modify/Reschedule/Manage → `/setup` / `/booking`; **Myself → `/`**. No managed commute → "Explore the managed commute" → `/eligibility`. `TabBar`. Also updated screen 10's Clearline toggle: a J2-completed user gets "Open my managed commute" → `/managed` (probes `/api/managed`). `tests/managed.test.tsx` (3).
- **P10.2 — 22 Live trip** (`/trip`) ✅ 🔒 — `TripMap` band + "Live · updated now" pill, compact `SplitFlap` ETA, driver/vehicle (labelled sample data), **step tracker** (done/now/upcoming, "now" heavier ink), held-vehicle line, Share (copies link), **Emergency (oxblood + honest copy)**. Subscribes to **Supabase Realtime** UPDATEs on `managed_trips` + 30s re-read so the schedule-derived tracker advances with no feed. `GET /api/managed/trip` creates today's row (service-role, client read-only + Realtime source), owned via booking→setup (RLS); step state SCHEDULE-DERIVED, never fabricated GPS; source via `LIVE_FULFILMENT_SOURCE` (Tookan-trial ↔ DIY). `supabase/migrations/…0004_realtime.sql` adds `managed_trips` to the realtime publication (RLS still gates delivery). Added `phone` icon. `tests/trip.test.tsx` (4).

**Phase 10 checkpoint — ALL 24 SCREENS BUILT.** Both journeys run end to end on the local build: J1 login→plan→ways→detail→home→More; J2 eligibility→(corridor/commit or straight-to-setup)→setup→itinerary→booking→managed-home→live-trip. Honesty labels throughout; RLS + k-anon hold; no money wired; `billing_on` unforgeable; Realtime wired for the live trip. 137 Vitest + 27 pgTAP green. ▶ Phase 11 (hardening / a11y / PWA / e2e / dev-route gate / full security sweep) next.

## Phase 11 — Hardening / a11y / PWA / launch prep

- **P11.1 — `/dev/*` gated** — `lib/supabase/middleware.ts`: the dev galleries return 404 in production (`NODE_ENV`) and leave the public-path allowlist. (Was public + shipped — PARKED.)
- **P11.2 — Honest offline banner** — `components/offline-banner.tsx`, mounted in `AppShell` for every screen. Offline → oxblood `role="status"` saying the times shown are the last ones loaded and may be stale; never implies live; renders nothing online; reacts to `online`/`offline`. `tests/offline-banner.test.tsx` (3).
- **P11.3 — a11y static pass** — icon-only buttons all carry `aria-label`; inputs use `TextField` labels; `:focus-visible` ink ring + `prefers-reduced-motion` settles are global; live banners are `role="status"`. A live axe run is deferred to the on-device walkthrough.
- **P11.4 — Full security-review sweep** (self-review; no git remote) — all 12 API routes auth-gated, `user_id` from the session + RLS backstop; service-role only post-auth on no-client-write tables, scoped to the caller; `partnered` + `billing_on` server-derived and test-proven unforgeable; k-anon N≥5 on demand; home never in a signal; no payment path; no secrets in responses; `.env*` gitignored. **Zero high-severity.** Advisories → PARKED: API rate-limiting, prod SMS/OTP, the `SECURITY DEFINER` k-anon fn (intended, anon revoked).
- **P11.5 — Playwright e2e** — `playwright.config.ts` + `tests/e2e/journeys.spec.ts` cover both full journeys (incl. asserting `₹0 · pilot`), `pnpm test:e2e` wired. tsconfig/eslint exclude the e2e dir so unit checks stay green. **DONE 2026-09-04 (commit `b98d194`):** installed `@playwright/test` 1.62.1 + chromium; added `tests/e2e/auth.setup.ts` (logs in once → `storageState`, reused by a `setup` project dependency so the run makes a single OTP send — the per-number SMS-frequency limit was flaking re-auth); reconciled the spec to the built UI (it had been written from `journey.md` and never run — OTP now types into the first `.otp` cell and lets the component auto-advance/auto-submit; post-verify lands on `/`; real copy for Search start/destination, "Set as my commute", work-email, "/ 250 committed", Home area/Tower, "Accept & save this commute"). **5/5 green, two consecutive clean runs.**
- **P11.6 — Prod build** — `next build` clean: all 24 routes compile, middleware 62.5 kB, first-load JS ~105–149 kB.
- **P11.7 — Live-walkthrough fixes** — batched Chrome walkthrough of both journeys (login→plan→save; J2 eligibility→corridor→commit→setup→itinerary→booking→managed→trip), console clean on every screen. Fixed two CSS class-name collisions found live: `.trip` (privacy rows vs screen-08 masthead trip-line → `.priv-trip`), `.empty` (onboarding vs the prototype's `.val.empty` setrow → `.onboard`); and gave the J2 itinerary's wider "SHUTTLE" mode label its own column (`.mgd-legs .r`, J1 v8 untouched). Re-verified live.

**Phase 11 checkpoint — BUILD COMPLETE.** All 24 screens live and verified in a real browser; both journeys pass end-to-end on real Supabase data; honesty labels everywhere; RLS + k-anon (27 pgTAP) hold; no money wired; `billing_on` unforgeable; Realtime wired; PWA installable with honest offline; `/dev/*` gated for prod. 140 Vitest + 27 pgTAP green; typecheck + lint + `next build` clean. **Human-gated remainder (PARKED): Playwright install + run, API rate-limiting, prod SMS/OTP config, remote Supabase keys, `git push` + Vercel deploy + on-device NO-GO walkthrough.**

## Post-build — 2026-09-04 (session: handoff pickup)

- **Playwright e2e done** (P11.5) — see the P11.5 entry above; `@playwright/test` 1.62.1 + chromium installed, `auth.setup.ts` shared-login, spec reconciled to the built UI, **5/5 green** (two clean runs).
- **Repo pushed to GitHub** — `github.com/TarunPur/traffic-congestion` (PUBLIC). Fresh local history force-pushed over the stub `Initial commit`. Needed a `workflow`-scoped `gh` token (Tarun granted it) to push `.github/workflows/ci.yml`. `.env.local` gitignored + whole tree scanned — no secrets committed. Remote Supabase service-role key must stay Vercel-only.
- **CI green** — GitHub Actions `verify` job (typecheck / lint / test / build) passes on push. pgTAP + Playwright not in CI (need a Postgres/Supabase service container — add during deploy setup if wanted).
- **CI-only bug fixed** (`fix(test)`, commit after the push) — `tests/corridor.test.tsx` returned a fresh `{push,replace}` from the `useRouter` mock each call; `CorridorPage`'s `useEffect(…, [router])` then re-ran on every render and a late `/api/corridor` fetch escaped `afterEach`'s `vi.unstubAllGlobals()`. On CI's **Node 22** undici rejects the relative URL → unhandled rejection fails the run (all 137 tests still passed; Node 26 locally tolerated it). Mock now returns a stable router object via `vi.hoisted` — matches real `useRouter()`. **Only `corridor` had a `[router]`-keyed effect; no other test file affected.**
- **Still PARKED / needs Tarun:** API rate-limiting · prod SMS/OTP · remote Supabase keys + `supabase db push` · Vercel deploy · on-device NO-GO walkthrough · Mappls key (planner still on stub) · `next@15.1.6` has CVE-2025-66478 (bump before public launch) · ERD §12 open decisions.

## Post-build — 2026-09-05 (session: Vercel + remote Supabase go-live, live QA pass)

- **🚀 LIVE: `https://traffic-congestion-tau.vercel.app`.** Sequence: remote Supabase project "Traffic congestion" (`sgwazmaiaydkgdizguxy`, Singapore, Postgres 17) → CLI linked → `supabase db push` (migrations 0001–0004) → seed data via SQL Editor (bulk `insert` doesn't run through `db push`) → Vercel env vars set (Production+Preview) → deployed.
- **CVE-2025-66478 fixed** (CVSS 10.0 RCE, all Next 15.x App Router apps) — bumped `next` 15.1.6→15.1.9. Vercel hard-blocks vulnerable-Next.js deploys; the failure showed only a generic "Build Failed" card, real cause was the *last line* of the build log. App was never publicly reachable while vulnerable (every deploy refused), so no secret rotation needed.
- **Deploy blocker: `NEXT_PUBLIC_*` env vars bake in at build time** — saving in the Vercel dashboard doesn't rebuild; needs an explicit redeploy. Also switched the two public Supabase vars Secret→Config type so they stay human-verifiable (service_role stays Secret).
- **Deploy blocker: remote Supabase had Phone auth disabled** — that's why only ~4 screens were reachable after first deploy (couldn't get past login). Enabled with the same test-number pattern as local (`919812345678=424242`, `919999900001=123456`, valid to 2027-09-30) + dummy Twilio creds.
- **Live QA pass on production** (not localhost) — 22/24 screens directly tested against real data, including two live database-write tests (corridor commit 0→1, managed booking→confirm). Full report: screenshots + functional + `design.md`-match findings for every screen. Cross-checked `design.md` token-for-token against the actual deployed CSS bundle (fetched and grepped, not eyeballed) — exact match on colors, `.tl.mgd`/`.tl.sched` markers, `.prog` bar, boarding-pass `.perf` perforation, focus ring, split-flap dimensions. Two minor letter-spacing deviations found (CTA `.055em` vs spec `.11em`; `.lab` `.17em` vs spec `.14em`) — cosmetic, not fixed, Tarun's call.
- **Two real bugs found + fixed during live QA** (both deployed, reverified live — neither caught by the test suite, jsdom does no layout and mocked fetch has no loading window):
  1. **Screen 10 home** — `.addrow .tx .t`/`.d` are `<span>`s missing `display:block`, so the evening-commute title and description ran together on one line. **The frozen prototype has the identical bug** — flagged in `PARKED.md`, prototype left untouched pending Tarun's sign-off.
  2. **Screen 17 corridor** — for 1–3s while `/api/corridor` loads, the page showed a hardcoded fallback corridor name (not even the real one) and a contradictory "threshold is met" message instead of a neutral loading state. Fixed: name falls back to "Loading…", the note explicitly checks for missing data first.
- **Side effect of QA:** the corridor commit counter is now genuinely at **1** (was 0) from live-testing the write path. Real data, not fake — but Tarun hadn't said whether to reset it to 0 as of this entry.
- Repo/GitHub/Vercel confirmed byte-identical at `8af139e` (hash-compared, not assumed); CI green.
- **Corrected course twice on deliverable format this session**: Tarun wanted the live Vercel URL to share with testers, not a screenshot artifact (that's a review aid, not the product); then wanted actual screenshots embedded in the QA report, not a text-only table. Full detail + current state in `HANDOFF-NEXT-AGENT.md` (rewritten this session — read it first).
- **Still PARKED / needs Tarun:** Mappls signup+key (biggest functional gap — planner is still 100% stub), API rate-limiting, real SMS provider for open testing, corridor-counter reset y/n, the two letter-spacing deviations y/n, full on-device NO-GO walkthrough, prototype `.addrow` bug fix y/n.
