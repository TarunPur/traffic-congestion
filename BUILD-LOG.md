...
- **P11.5 — Playwright e2e** — `playwright.config.ts` + `tests/e2e/journeys.spec.ts` cover both full journeys (incl. asserting `₹0 · pilot`), `pnpm test:e2e` wired. **Blocked on a dependency add** (`@playwright/test`) — pending approval, PARKED. tsconfig/eslint exclude the e2e dir so unit checks stay green.
